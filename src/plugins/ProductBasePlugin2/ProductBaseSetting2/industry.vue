 ...
 * @LastEditors: CiFong
 * @LastEditTime: 2020-07-30 11:47:41
 * @Description: 请输入
-->
<template>
    ...
    >
      ...
      >
        <el-form-item label="行业种类名称:" prop="name">
          <el-input v-model.trim="form.name" placeholder="请输入行业种类名称" />
        </el-form-item>
      </el-form>
      <span slot="footer">
...
export default {
  data() {
    return {
      ...
      formRules: {
        name: [{ required: true, message: "请输入行业种类名称", trigger: "blur" }]
      },
      // 弹窗 - 行业表单样式
      labelWidth: "120pR",
      labelPosition: "right",
  ...
  methods: {
    async init() {
      ...
      this.tableData = [];
      try {
        const data = await industryApi.findIndustry();
        data.forEach(item => {
          this.tableData.push({
            name: item.name,
            id: item.id
    ...
    async addHandle() {
      ...
      try {
        await industryApi.addIndustry(data);
    ...
    async editHandle() {
      ...
      try {
        await industryApi.editIndustry(data);
    ...
    async delHandle(row) {
      ...
      try {
        await industryApi.delIndustry(data);
