<template>
  <div id="businessDetail">
    <el-collapse v-model="pannel" accordion>
      <el-collapse-item v-for="i in business" :key="i.id" :name="i.id">
        <template slot="title">
          <div class="business-header">
            <span class="title">{{ i.title }}</span>
            <span class="star">
              <span
                v-for="(star, index2) in starRange(parseInt(i.recommendRate))"
                :key="index2 + '12'"
                class="el-icon-star-on recommend"
              >
              </span>
              <span
                v-for="(star, index2) in starRange(5 - parseInt(i.recommendRate))"
                :key="index2 + '22'"
                class="el-icon-star-on unrecommend"
              ></span>
            </span>
          </div>
        </template>
