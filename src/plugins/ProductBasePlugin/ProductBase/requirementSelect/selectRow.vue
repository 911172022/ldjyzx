      ...
      <el-collapse-item name='pannel'>
        <template slot='title'>
          <div class='select-row'>
            <label>{{ label }}：</label>
            <ul>
              <li v-for='item in selection.slice(0, selectionCount)' :key='item.Keyword' @click.stop='selectHandle(item)' :class="selectionClass(item)">
                {{ item.Code }}
              </li>
              <li v-show='selectionToggle' v-for='item in selection.slice(selectionCount, selection.length)' :key='item.Keyword' @click.stop='selectHandle(item)' :class="selectionClass(item)">
                {{ item.Code }}
              </li>
            </ul>
            <div class='more-btn' @click.stop='showMoreSelection' v-if='selection.length > selectionCount'>
        ...
        <div class='more-selection' v-loading='moreLoading' :style="moreLoading ? 'height: 40px;': 'height: unset'">
          <ul>
            <li v-for='item in moreSelection' :key='item.Keyword' @click="selectHandle2(item)" :class="moreSelectionClass(item)">
              {{ item.Code }}
            </li>
          </ul>
        </div>
...
export default {
  ...
  computed: {
    ...
    moreSelectionClass(){
      return function(item){
        return (this.moreSelectionSelected.Keyword === item.Keyword ? 'product-selected': '') + ' selection'
      }
    },
