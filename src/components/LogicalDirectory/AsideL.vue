<template>
  <div class="asideL">
    ...
    <TreeContextMenu :dialogObj="rightData" @reData="reData(arguments)" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
      ...
      if (
        ...
        if (res.success) {
          ...
          if (
            ...
            // 添加逻辑目录判断依据
            vm.rightData.isLogicalDirectory = true
            this.NewUserGroupChildren = undefined;
          }
        } else {
          this.$message({ message: res.msg, type: "error" });
        ...
        if (res.data[0].state === "addSuccess") {
          this.$message({ message: "已添加到收藏夹", type: "success" });
        }
    ...
    reData(e) {
      ...
      // 关掉 isLogicalDirectory
      this.rightData.isLogicalDirectory = false
    },
    openUser() {
    ...
    // 左侧选单 - 右键
    contextMenuClick(MouseEvent, object) {
      if (object) {
      ...
      this.$store.dispatch("dbsource/getMenuList", {
        ProjectList,
        Position,
        TvPosition
      });
      ...
      let aa =
        (MouseEvent.clientY + this.$el.getBoundingClientRect().height) / 1.5;
