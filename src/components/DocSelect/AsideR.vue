          ...
          <table class="isTable">
            ...
            <tr v-for="(item, index) in ProjectAttrData" :key="index">
              <td
                ...
                :style="{ cursor: item.CanEdit === 'True' ? 'pointer': 'unset'}"
              >
                {{ item.AttrName }}
        ...
        <el-tab-pane label="文档属性" name="c">
          <table class="isTable">
            ...
            <tr v-for="(item, index) in DocBaseAttr" :key="index">
              <td class="bgTd" v-if="item.AttrName === '文件名'" style="cursor: pointer;">
                <menuUpload :uploadArr="UploadFiles" @reData="reFiles">
                  <slot>
                    {{ item.AttrName }}
        ...
        <el-tab-pane label="文档_附加属性" name="d">
          <table class="isTable">
            ...
            <tr v-for="(item, index) in DocAttrData" :key="index">
              <td class="bgTd" @click="openEditAttributes(item, 'Doc')" :style="{ cursor: item.CanEdit === 'True' ? 'pointer': 'unset'}">
                {{ item.AttrName }}
                <i class="el-icon-edit"  v-if="item.CanEdit === 'True'" />
              </td>
              <template v-if="editAttributesData.AttrName !== item.AttrName">
      ...
      <template v-if="tabsName.isAdmin">
        <el-tab-pane label="权限列表" name="g">
          <div class="isBtn">
            ...
            >增加</el-button>
            <el-button size="small" type="primary" icon="el-icon-delete" @click="openRemoveUser">删除</el-button>
            <el-button size="small" type="primary" icon="el-icon-refresh" @click="editData">保存</el-button>
          </div>
          ...
          >
            <el-table-column prop="UserName" label="用户/用户组" width="200" show-overflow-tooltip />
            <el-table-column prop="AcceObj" label="权限所在目录" min-width="180" />
            ...
            <el-table-column label="创建" width="100" align="center">
              <template slot-scope="scope">
                <el-checkbox
                  ...
                  @change="directoryCheckedCreate(scope.$index, scope.row.PCreate)"
                />
              </template>
            </el-table-column>
            ...
            <el-table-column label="写" width="100" align="center">
              <template slot-scope="scope">
                <el-checkbox
                  ...
                  @change="directoryCheckedWrite(scope.$index, scope.row.PWrite)"
                />
              </template>
            </el-table-column>
            <el-table-column label="删除" width="100" align="center">
              <template slot-scope="scope">
                <el-checkbox
                  ...
                  @change="directoryCheckedDelete(scope.$index, scope.row.PDelete)"
                />
              </template>
            </el-table-column>
            <el-table-column label="控制" width="100" align="center">
              <template slot-scope="scope">
                <el-checkbox
                  ...
                  @change="directoryCheckedControl(scope.$index, scope.row.PCntrl)"
                />
              </template>
            </el-table-column>
            <el-table-column label="无" width="100" align="center">
              <template slot-scope="scope">
                <el-checkbox
                  ...
                  @change="directoryCheckedCancel(scope.$index, scope.row.PNone)"
                />
              </template>
            </el-table-column>
          </el-table>
            ...
            <el-table-column label="写文件" width="100" align="center">
              <template slot-scope="scope">
                <el-checkbox
                  ...
                  @change="fileCheckedWriteFile(scope.$index, scope.row.DFWrite)"
                />
              </template>
            </el-table-column>
            <el-table-column label="控制" width="100" align="center">
  ...
  methods: {
    ...
    async PreviewHandle() {
      ...
      if (res.success) {
        ...
        } else if (str === "PNG" || str === "JPG" || str === "JPEG" || str === 'JPGE') {
          // this.isPreview = true;
        ...
        } else if (str === "MP4" || str === "WMV" || str === 'AVI') {
          let RefDocKeyword = ""
          const res2 = await UserApi3.fileDownload(this.DocKeyword, RefDocKeyword)
          this.$store.commit("menu/GET_NAVURL", "");
    ...
    async reTemplate(e) {
      let vm = this,
        ...
        docAttrJson = [],
        res = null;
      docAttrJson.push({ name: "tempDefnId", value: e.defkeyid, attrtype: "" });
      if (e.type) {
        if (e.type === "Doc") {
