      ...
      <div class="tasks-wrapper" ref="tasksWrapper" id="tasksWrapper">
        ...
        >
          ...
          <div class="flex-column info-wrapper">
            ...
            <div class="file-path">
              {{ i.downloadedSize }} / {{ i.size }} （{{ tasksStatus(i) }}）
            </div>
            <div>
              <el-progress
                :text-inside="true"
                :percentage="i.status === 'Done' ? 100 : parseInt(i.progress)"
                :stroke-width="18"
  ...
  methods: {
    ...
    test() {
      ...
      let data = {
        ...
        url:
          "http://jhcdms.f3322.net:81/GJEPCStorage/CDMS0006083/testZip%20-%20%e5%89%af%e6%9c%ac.zip?p=AxGa3x39tENVQ0DTfmcUE/pVjn4RdXDGxJ1aat2/T70IT9xFOgTfgbHFcTs1vPt9WQ9/QjVNXGNOnLR7y6/YY9Vfdl1b/cc1UVuGIZVIGETCpnra%2B8jHbz83AtQ%2BSprBsgEeZuMR77f37QPmQHBCwDYqjkoy64j6sGGHfC8/Vxg=",
        progress: "100%",
  ...
  watch: {
    ...
    downloadTasks: {
      handler(newValue, oldValue) {
        ...
        if (
          newValue.filter((i) => i.status === "Done").length === newValue.length
        ) {
          this.tasksTrigger = false;
        }
      },
