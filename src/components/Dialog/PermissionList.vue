<template>
  <el-dialog :append-to-body='appendToBody' class="addUser" :title="dialogObjtitle.title" :visible.sync="dialogObj.switch" width="60%" :before-close="dialogClose">
    <el-row :gutter="20">
      <el-col :span="12">
  ...
  methods: {
    ...
    addUserHandle () {
      ...
      if (!arr.length) {
        ...
        // vm.$emit("user", arr, vm.dialogObj.text)
        // vm.$store.dispatch('workflow/addWorkUser', { ObjectKeyword, WorkStateKeyword, UserList })
        vm.$message({
          message: '添加成功',
          type: 'success'
        })
