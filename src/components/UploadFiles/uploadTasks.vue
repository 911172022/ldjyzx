...
<template>
    <div id="uploadTasks" :draggable="false">
        ...
        <!-- @mousedown="move" -->
        <div class="ball" v-show="true" @mousedown="move" :style="ballStyle"  @click="showTasks" ref='ball'>
            <div class='el-icon-upload2 download-icon'></div>
        </div>
        <div class='wrapper' :style="wrapperStyle">
            <div class="tasks-wrapper" ref='tasksWrapper' id="tasksWrapper">
                <div v-for="(i, index) in uploadList" :key="i.keyword + index" class="tasks-card flex-row" :style="cardStyle">
                    <div class="logo-wrapper">
                        <img :src='AddTypeIcon(i.O_filename)' />
                    </div>
                    <div class="flex-column info-wrapper">
                        <div class="file-name">{{ i.O_filename }}  <span class="tip">（{{ statusTip(i.status )}}）</span></div>
                        <!-- <div class="file-path">{{ statusTip(i.status )}}</div> -->
                        <div class="progress-wrapper">
                            <div style="flex: 1;">
                                <el-progress :text-inside="true" :percentage="i.progressPercent" :stroke-width="18" :color="progressColor(i)"/>
                            </div>
                            <div>
                                <i v-if="i.status === tasksStatus.UPLOADING || i.status === tasksStatus.WAITING_UPLOAD" @click="pauseTasks(i)" class="el-icon-video-pause"></i>
                                <i v-if='i.status === tasksStatus.PAUSE' @click="restartTasks(i)" class="el-icon-refresh-right"></i>
                            </div>
                        </div>
                        <div class="speed-tip" v-if="i.status === tasksStatus.UPLOADING">上传速度：{{ uploadSpeed | filterSize }} / s  ， 共{{ i.rawFile.size | filterSize }}</div>
                        <div class="err-tip" v-if="i.status === tasksStatus.REMOVE || i.status === tasksStatus.PAUSE">
                            “{{ i.msg }}”
                        </div>
                    </div>
                </div>
