  ...
  <div>
    ...
    >
      <span style="text-align: center"
        >目录下已存在同名文件是否升级或覆盖?</span
      >
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="confirmHandle('true')"
          >升级</el-button
        >
        <el-button
          type="primary"
          @click="
            confirmAllHandle('allUpgrade');
            confirmHandle('true');
          "
          >全部升级</el-button
        >
        <el-button type="primary" @click="confirmHandle('false')"
          >覆盖</el-button
        >
        <el-button
          type="primary"
          @click="
            confirmAllHandle('allCover');
            confirmHandle('false');
          "
          >全部覆盖</el-button
        >
...
export default {
  ...
  methods: {
    ...
    //   let vm = this,
    //     { nDuplicateFiles } = vm,
    //     { nDuplicateKeywords } = vm,
    //     len = nDuplicateFiles.length;
    //   for (let i = 0; i < len; i++) {
    ...
    //         // 在收文文件夹上传时 触发
    //         UserApi3.afterCreateNewDocEvent(nDuplicateKeywords[i]).then(
    //           (res) => {
