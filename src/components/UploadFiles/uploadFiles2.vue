        ...
        >
            ...
            <div class="uploadTaskWrapper">
                <table>
                    ...
                    <!-- v-show="!item.pause" -->
                    <tr v-for="(item, index) in fileList" :key="index" v-show="!item.remove">
                        <td>
                ...
                <div class="upload-info">
                    <div>共{{ uploadQueueLength }}个文件，已上传{{ uploadCompletedLength }}个文件。</div>
            ...
            >
                ...
                <span slot="footer" class="dialog-footer">
                    <el-button type="primary" @click="confirmHandle('upgrade')">升级</el-button>
                    <el-button type="primary" @click="confirmHandle('allUpgrade')">全部升级</el-button>
                    <el-button type="primary" @click="confirmHandle('cover')">覆盖</el-button>
                    <el-button type="primary" @click="confirmHandle('allCover')">全部覆盖</el-button>
                </span>
            </el-dialog>
            <!-- 确认关闭上传弹窗 -->
            <el-dialog
            ...
            get() {
                ...
                for (let i in this.fileList) {
                    ...
                    if (
                        ...
                        // 只要任务不是被移除，和已完成的，就不关
                        !this.fileList[i].remove && !this.fileList[i].completed
                    ) {
                        return true;
                    }
                }
    ...
    methods: {
        ...
        async checkDuplicate() {
            for (let i in this.fileList) {
                ...
                if (this.fileList[i].pause || this.fileList[i].remove || this.fileList[i].completed) {
                    continue;
                }
        ...
        async recordDuplicateInfo() {
            for (let i in this.fileList) {
                ...
                if (this.fileList[i].pause || !this.fileList[i].isDuplicate || this.fileList[i].remove) {
                    continue;
                }
            ...
            for (let i in this.fileList) {
                ...
                try {
                    ...
                    while (cursor < this.fileList[i].rawFile.size) {
                        fileChunks.push({
                            file: this.fileList[i].rawFile.raw.slice(
                                cursor,
                                cursor + SIZE
                            ),
                            uploaded:
                                this.fileList[i].fileUploadedSize > cursor
                                    ? true
                                    : false
                        });
                        cursor += SIZE;
                    ...
                    for (let re = 0; re < 10; re++) {
                        ...
                        let res = await uploadApi
                            .uploadFile(
                                ...
                                (progressEvent) => {
                                    ...
                                    let progressWrite = parseInt(((j  * SIZE + progressEvent.loaded) / this.fileList[i].rawFile.size) * 100)
                                    ...
                                    if (this.fileList[i].progressPercent >= progressWrite) {
                                        ...
                                        // 正常的上传
                                        this.fileList[i].progressPercent = this.fileList[i].progressPercent <= progressWrite ? progressWrite : this.fileList[i].progressPercent
                        ...
                        if (res) {
                            if (res.success) {
                                ...
                                // 计算进度条数据
                                this.fileList[i].progressPercent = this.progressPercent(this.fileList[i].fileChunks);
            ...
            if (res.success) {
                if (res.data[0].state === "uploadSuccess") {
                    ...
                    if (res2.success) {
                        // 收文弹窗
                        if (
                            res2.msg === "GetDocument" ||
                            res2.msg === "FyGetDocument"
                        ) {
                    ...
                    this.fileList[i].pause = true;
                    this.fileList[i].msg =  `${this.fileList[i].O_filename}上传完回调失败：${res.msg}`
                    this.$message({
