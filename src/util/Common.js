/*
 * @Author: CiFong
 * @Date: 2020-04-03 14:08:35
 * @LastEditors: CiFong
 * @LastEditTime: 2020-04-03 15:15:55
 * @Description: 请输入
 */

import store from '../store/index.js'
import { client_download_url, client_name } from '../const.js'
...
export function windowDownloadFile(sUrl, docKeyword = '') {
    //完全使用客户端下载的模式
    if (store.getters['home/downloadMode']) {
        const sid = store.getters.sid
        sUrl = 'cdmsmews://funcname~~~' + 'downloadfile' + '~~~' + 'dockeyword' + '~~~' + docKeyword + '~~~' + 'sid'+'~~~'+ sid+'~~~' +'baseurl'+'~~~' +BASE_URL;
        ...
        if (window.require) {
            ...
            //发送消息给客户端，让客户端下载
            var downloadData = { funcname: "downloadfile", dockeyword: docKeyword, sid };
            if (store.getters['home/loadIpcRenderer'] == false) {
                const { ipcRenderer } = window.require('electron');
                store.dispatch("home/setLoadIpcRenderer", true);
                window.ipcr = ipcRenderer;
            }

            if (window.ipcr) {
                //window.ipcr.send(downloadData);
                window.ipcr.sendToHost(downloadData) // 向原生发送信息
                window.ipcr.once("setDocCheckOut", (event, data) => {
                    console.log(event);
                    console.log(data);
                    store.dispatch({ type: 'home/setCheckOut', docKeyword: docKeyword })
                    //alert('接收electron原生返回的once信息'+'docKeyword:'+docKeyword)
                });
            }
            return 
            ...
            let downloadClient=function() {

                let result = confirm('windows协议未注册，请下载并安装插件！')
                if (result) {
                
                    let a = document.createElement('a')
                    a.download = client_name
            ...
            if (window.cdmswsonline=="protocolCheck")
            {
                alert('请刷新页面再打开文件！！')
            }

            //console.log('cdmswsonline:'+window.cdmswsonline+",aa:"+window.cdmsws);
            if(window.cdmswsonline==undefined || window.cdmswsonline=="false"){

                window.cdmswsonline="protocolCheck";

                //检查是否注册了window自定义协议，如果未注册，就安装插件，
                //如果已注册，就Window自定义协议
                window.protocolCheck(sUrl,downloadClient); 
            }else{
                //检查是否注册了window自定义协议，如果未注册，就安装插件，
                //如果已注册，就不触发任何事件，不调用Window自定义协议
                window.cdmsprotocolCheck(sUrl,downloadClient);

            }

            if (window.cdmsws){
                window.cdmsws.send(sUrl); 
                return;
            }else{
                alert('请刷新页面！')
            }
